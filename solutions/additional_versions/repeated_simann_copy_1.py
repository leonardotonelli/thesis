...
# DELTA REFERENCE BASED ON THE SUM OF THE DISTANCES WITH OTHER REPLICAS
# COMPUTE_DELTA_COST() NOT OPTIMIZED
...
class BinaryPerceptronRepeated:
    ...
    def compute_delta_cost(self, action, replicas_weights):
        '''
        Compute delta cost of a given action efficiently
        '''

        # current pred
        distance1 = self.compute_distance(replicas_weights)
        starting_cost = self.compute_cost()

        # new
        temp_problem = self.copy()
        temp_problem.accept_action(action)  
        distance2 = temp_problem.compute_distance(replicas_weights)
        new_cost = temp_problem.compute_cost()

        # separated deltas
        delta_energy = (new_cost - starting_cost)
        delta_reference = distance2 - distance1
    ...
    def compute_distance(self, replicas_weights):
        ''' 
        Function that computes the distance between the given replica to the reference
        '''
        d = np.sum( ( ( replicas_weights - self.weights.reshape(-1, 1) )**2 ) / 2 ) # broadcast the distance
        return d
...
class RepeatedSimann:
    ...
    def compute_deltas(self, replica_index, action):
        ...
        delta_energy, delta_reference = self.replicas[replica_index].compute_delta_cost(action, self.replicas_weights)
        return delta_energy, delta_reference
