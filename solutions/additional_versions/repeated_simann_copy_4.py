...
# SAME AS THE MAIN, BUT WITH DISCRETE REFERENCE
...
class BinaryPerceptronRepeated:
    ...
    def compute_delta_cost(self, action, reference_weights):
        ...
        # compute delta
        delta_energy = float(new_cost - current_cost)
        delta_reference = float(2*self.weights[action]*reference_weights[action])
        ...
        delta_energy_real = float(new_cost - starting_cost)
        delta_reference_real = float(distance2 - distance1)

        # check whether the new delta cost is working
        assert round(delta_energy,2) == round(delta_energy_real,2)
        assert round(delta_reference,2) == round(delta_reference_real,2)
    ...
    def compute_distance(self, reference_weights):
        ...
        d = np.sum((reference_weights - self.weights)**2)/2 # it should be broadcasted
...
class RepeatedSimann:
    ...
    def compute_reference(self):
        ...
        '''
        ref = np.sign(self.replicas_weights.sum(axis=1))
        self.reference = ref
