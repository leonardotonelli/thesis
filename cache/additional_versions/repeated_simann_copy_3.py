...
# WITH DELTA REFERENCE COMPUTED FROM THE EQUATION 2 OF S6. HERE DELTA REFERENCE MUST INCLUDE GAMMA.
...
class BinaryPerceptronRepeated:
    ...
    def compute_delta_cost(self, action, replicas_weights, replica_index, gamma):
        '''
        ...
        replicas_weights_new = replicas_weights.copy()
        replicas_weights_new[action, replica_index] = -replicas_weights_new[action, replica_index]
        delta_reference_w_gamma = np.log(2*np.cosh(gamma* np.sum(replicas_weights[action,:]))) - np.log(2*np.cosh(gamma* np.sum(replicas_weights_new[action,:])))

        return delta_energy, delta_reference_w_gamma
...
class RepeatedSimann:
    ...
    def compute_deltas(self, replica_index, action, gamma):
        '''
        compute delta cost for the given replica, move and the reference weights
        '''
        # reference_weights = self.reference
        replicas_weights = self.replicas_weights
        delta_energy, delta_reference_w_gamma = self.replicas[replica_index].compute_delta_cost(action, replicas_weights, replica_index, gamma)
        return delta_energy, delta_reference_w_gamma
    ...
    for i in range(annealing_steps): # assume that annealing steps are the same for scooping for now
        ...
        for t in range(mcmc_steps):
            ...
            for replica_index, replica in enumerate(probl.replicas):
                ...
                # compute delta cost for the given replica, move and the reference weights
                deltas = probl.compute_deltas(replica_index, action, gammas[i])

                if accept_with_prob(deltas, betas[i]):
                    accepted_moves += 1
...
def accept_with_prob(deltas, beta):
    delta_energy, delta_reference_w_gamma = deltas
    if delta_energy <= 0:
        return True
    if beta == np.inf:
        return False
    ...
    prob = np.exp(-beta * delta_energy + delta_reference_w_gamma)
